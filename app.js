...
function taskRow(t,ctx){
  ...
  return `<div class="task2 ${t.done?'done':''}" data-task="${t.id}">
    ...
    <div class="t2-body" onclick="toggleTask('${t.id}')">
...
function openGoal(id){
  ...
  $('goal-due').value=curGoal.due||'';
  $('goal-rem-sw').classList.toggle('on',!!curGoal.remOn);
  $('goal-rem-time').value=curGoal.remTime||'19:00';
  renderSteps();
  openSheet('goal-ov',()=>{
    if(curGoal){
      saveGoalLive();
...
function goalRemToggle(){
  ...
  if(curGoal.remOn && !curGoal.remTime) curGoal.remTime=$('goal-rem-time').value||'19:00';
  $('goal-rem-sw').classList.toggle('on',curGoal.remOn);
  saveGoalLive();
...
function saveGoalLive(){
  ...
  curGoal.due=$('goal-due').value;
  curGoal.remTime=$('goal-rem-time').value||'19:00';
  save();
}
...
['goal-em','goal-name','goal-why','goal-due'].forEach(id=>{
...
function openCatSheet(){
  ...
  openSheet('cat-sheet',()=>{
    const bg=$('half-bg'); bg.classList.add('closing');
    setTimeout(()=>bg.classList.remove('on','closing'),200);
  });
}
